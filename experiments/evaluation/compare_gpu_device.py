...
def compute_scores_gpu(compressor, instruction: str, query: str, chunks: List[str], batch_size: int = 32) -> List[float]:
    """
...
def generate_summary_report(results: List[Dict], output_file: str, top_ks: List[int]):
    ...
    for k in top_ks:
        agreements = [r["top_k_agreements"].get(k, None) for r in results if r["top_k_agreements"].get(k, None) is not None]
        if agreements:
...
def main():
    ...
    parser.add_argument("--top_ks", type=str, default="10%,20%,50%", help="要评估的Top-K值，用逗号分隔，可以是整数或百分比 (例如: 1,3,5 或 10%,20%,50%)")
