...
class PPLCompressor(BaseCompressor):
    def __init__(
        self,
        model_name: str,
        device,
    ):
        super().__init__("PPLCompressor")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
    ...
    def get_ppl(
        ...
        end = min(end, self.max_position_embeddings)
        with torch.no_grad():
            response = self.model(input_ids, attention_mask=attention_mask, use_cache=False)

        shift_logits = response.logits[..., :-1, :].contiguous()  # [1, seq_len-1, vocab_size]
        shift_labels = input_ids[..., 1:end].contiguous()  # [1, end-1]
        shift_attention = attention_mask[..., 1:end].contiguous()  # [1, end-1]
        # Flatten for easier slicing
        flat_logits = shift_logits.view(-1, shift_logits.size(-1))  # [seq_len-1, vocab_size]
        flat_labels = shift_labels.view(-1)  # [seq_len-1]
        flat_mask = shift_attention.view(-1)  # [seq_len-1]
        loss_start = max(loss_start - 1, 0)
        loss_end = loss_end - 1
        print(loss_start)
        print(loss_end)
        print(flat_logits.shape)
        print(flat_labels.shape)
        flat_logits = flat_logits[loss_start:loss_end, :]
        flat_labels = flat_labels[loss_start:loss_end]

        loss_fct = torch.nn.CrossEntropyLoss(reduction="none")

        def chunked_loss(logits: torch.Tensor, targets: torch.Tensor, chunk_size: int = 2048) -> torch.Tensor:
            """
            Compute cross-entropy loss in chunks to save memory.

            Args:
                logits: [N, vocab_size] float tensor.
                targets: [N] long tensor.
                chunk_size: number of samples per chunk.
            Returns:
                [N] float tensor of per-token losses.
            """
            losses = []
            total = logits.size(0)
            for i in range(0, total, chunk_size):
                chunk_logits = logits[i : i + chunk_size]  # [chunk, vocab_size]
                chunk_targets = targets[i : i + chunk_size]  # [chunk]
                log_probs = F.log_softmax(chunk_logits, dim=-1)  # [chunk, vocab_size]
                loss = -log_probs[
                    torch.arange(chunk_targets.size(0), device=chunk_logits.device),
                    chunk_targets,
                ]
                losses.append(loss)

            return torch.cat(losses, dim=0)  # [N]

        with torch.no_grad():
            print(flat_logits.shape)
            print(flat_labels.shape)
            loss = chunked_loss(flat_logits, flat_labels)  # [seq_len-1]
        ppl_chunks = []
        total = loss.size(0)
        for i in range(0, total, chunk_size):
            chunk = loss[i : i + chunk_size]
            if chunk.numel() == 0:
                continue
            chunk_loss = chunk.mean()
            ppl_chunks.append(chunk_loss)
        output = torch.stack(ppl_chunks)
        return output
    ...
    def compress(
        ...
        with torch.no_grad():
            prefix_text = text[:document_start]
            document_text = text[document_start : document_start + document_len]

            prefix_ids = self.tokenizer(prefix_text, return_tensors="pt")["input_ids"].to(self.device)
            document_ids = self.tokenizer(document_text, return_tensors="pt")["input_ids"].to(self.device)

        document_token_start = prefix_ids.shape[1]
        document_token_end = document_token_start + document_ids.shape[1]
        ...
        ppls_tensor = self.get_ppl(
            ...
            loss_start=document_token_start - 1,  # 对应 shifted label
            loss_end=document_token_end - 1,
        )
...
class AttnScoreCompressor(BaseCompressor):
    def __init__(
        self,
        model_name: str,
        device_map,
    ):
        super().__init__("AttnScoreCompressor")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.model = AutoModelForCausalLM.from_pretrained(
            model_name,
            attn_implementation="flash_attention_2",
        ...
        self.model.to(torch.device(device_map))
    ...
    def get_score(
        ...
        hiddens = response.hidden_states

        with torch.no_grad():
            q_proj = self.model.model.layers[27].self_attn.q_proj
            hidden = hiddens[27]
            q = q_proj(hidden)
            k = past_key_values[27][0]

            del hiddens, past_key_values
            gc.collect()
            torch.cuda.empty_cache()

            batch_size, n_kv_head, seqlen, head_dim = k.shape
            print(seqlen)
            n_kv_group = q.shape[-1] // (n_kv_head * head_dim)
            n_head = n_kv_head * n_kv_group
            q = q.view(batch_size, seqlen, n_head, head_dim).transpose(1, 2)
            k = k.repeat_interleave(n_kv_group, dim=1)
            q_last = q[:, :, -chunk_size:, :]  # 只取最后 chunk_size 的 query
            attn_scores = torch.matmul(q_last, k.transpose(-2, -1)) / math.sqrt(head_dim)
            attn_scores = torch.softmax(attn_scores, dim=-1)

        attn_scores = attn_scores.sum(dim=1)  # [1, seq_len, seq_len]
        print(attn_scores.shape)
        attn_scores = attn_scores[:, -chunk_size:, :-chunk_size].sum(1).unsqueeze(0)
        print(attn_scores.shape)
        pad_len = (chunk_size - attn_scores.shape[-1] % chunk_size) % chunk_size
        attn_scores = F.pad(attn_scores, (0, pad_len))
        if attn_scores.shape[-1] >= chunk_size:
            scores = F.avg_pool1d(
                attn_scores,
            ...
            scores = torch.tensor([], dtype=torch.float32, device=attn_scores.device)
        del attn_scores, q, k, hidden
        gc.collect()
        torch.cuda.empty_cache()
    ...
    def compress(
        ...
        document_token_end = input_ids.shape[1] - chunk_size

        scores_tensor = self.get_score(
            text=None,
            input_ids=input_ids,
            attention_mask=attention_mask,
            chunk_size=chunk_size,
        ...
        input_ids = input_ids.squeeze(0)
        document_ids_1d = input_ids[document_token_start:document_token_end]
        keep_n = max(0, int(scores_tensor.shape[0] * rate) - 1)
        # sorted_indices = sorted(range(len(ppls)), key=lambda i: ppls[i])
        topk_indices = torch.argsort(scores_tensor, descending=True)[:keep_n]
        selected_indices, _ = torch.sort(topk_indices)
        # last_chunk_idx = torch.tensor([scores_tensor.shape[0] - 1], device=selected_indices.device)
        # selected_indices = torch.cat([selected_indices, last_chunk_idx])
        compressed_ids = []
...
class RerankCompressor(BaseCompressor):
    def __init__(
        self,
        ...
        if engine == "hf":
            ...
            os.environ["CUDA_VISIBLE_DEVICES"] = device_map.split(":")[1]
            self.model = LLM(
                model=model_name,
                task="score",
                hf_overrides={
...
class LongLLMLinguaTokenCompressor(BaseCompressor):
    def __init__(
        ...
        super().__init__("LongLLMLinguaTokenCompressor")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=trust_remote_code)
    ...
    def to_sentences(self, paragraph):
        ...
        for sent in re.findall("[^!?。\.\!\?]+[!?。\.\!\?]?", paragraph, flags=re.U):
            sent_tokenize_list.append(sent)
    ...
    def get_threshold(self, ppl: torch.Tensor, ratio: float, condition_flag: bool = False):
        if ratio == 1.0:
            return float("-inf")
    ...
    def get_compress_input(self, diff_loss, input_ids, attention_mask, threshold):
        need_idx = torch.cat([torch.tensor([True], device=diff_loss.device), diff_loss > threshold])
    ...
    def iterative_sent_compress(
        ...
        prefixed_texts = prefix + context if isinstance(prefix, list) else [prefix] + context
        processed_texts = [prefixed_texts[0]]

        for paragraph in prefixed_texts[1:]:
            sentences = self.to_sentences(paragraph)
            processed_texts.extend(sentences)

        first_tokenized = [
            self.tokenizer(prefixed_texts[0], return_tensors="pt", add_special_tokens=False).to(self.device)
        ]

        other_tokenized = [
            [
                self.tokenizer(sent, return_tensors="pt", add_special_tokens=False).to(self.device)
                for sent in self.to_sentences(paragraph)
            ]
            for paragraph in processed_texts[1:]
        ]

        first_input_ids = [first_tokenized[0]["input_ids"]]
        first_attention_mask = [first_tokenized[0]["attention_mask"]]

        other_input_ids = [[sent["input_ids"] for sent in chunk_sentences] for chunk_sentences in other_tokenized]
        other_attention_mask = [
            [sent["attention_mask"] for sent in chunk_sentences] for chunk_sentences in other_tokenized
        ]

        all_input_ids = first_input_ids + other_input_ids
        all_attention_mask = first_attention_mask + other_attention_mask

        chunked_input_ids = []
        chunked_attention_mask = []

        for chunk_sent_ids, chunk_sent_mask in zip(other_input_ids, other_attention_mask):
            # 将 chunk 内所有句子的 input_ids 在最后一个维度拼接
            chunk_ids = torch.cat(chunk_sent_ids, dim=-1)
            chunk_mask = torch.cat(chunk_sent_mask, dim=-1)
            chunked_input_ids.append(chunk_ids)
            chunked_attention_mask.append(chunk_mask)

        # 将 first chunk 添加到前面
        all_chunked_input_ids = first_input_ids + chunked_input_ids
        context_len = [chunks.shape[1] for chunks in all_chunked_input_ids]
        compressed_input_ids = []
        end = 0
        past_key_values = None
        self_past_key_values = None
        past_compressed_input_ids = None  # 记录上一个压缩后的输入，和当前拼接
        past_compressed_attention_mask = None
        for context_id in range(len(all_input_ids)):
            if context_id == 0:
                end += all_input_ids[context_id].shape[1]
                response = self.model(
                    all_input_ids[0],
            ...
            sent_loss = []
            self_sent_loss = []
            for sent_id in range(len(all_input_ids[context_id])):
                end += all_input_ids[context_id][sent_id].shape[1]
                if (
                    past_compressed_input_ids is not None
                    and past_compressed_attention_mask is not None
                    and sent_id == 0
                ):
                    past_compressed_cur_input_ids = torch.cat(
                        (past_compressed_input_ids, all_input_ids[context_id][sent_id]),
                        dim=-1,
                    )
                    past_compressed_cur_attention_mask = torch.cat(
                        (
                            past_compressed_attention_mask,
                            all_attention_mask[context_id][sent_id],
                        ),
                        dim=-1,
                    )
                    cal_start = len(past_compressed_input_ids[0])
                    cal_end = len(past_compressed_cur_input_ids[0])
                else:
                    cal_start = None
                    cal_end = None
                    past_compressed_cur_input_ids = all_input_ids[context_id][sent_id]
                    past_compressed_cur_attention_mask = all_attention_mask[context_id][sent_id]

                loss, past_key_values = self.get_ppl(
                    "",
                    "sentence",
                    past_compressed_cur_input_ids,
                    past_compressed_cur_attention_mask,
                    past_key_values,
                    start=cal_start,
                    end=cal_end,
                    return_kv=True,
                ...
                self_sent_loss.append(self_loss)

            diff_ppl = torch.Tensor(self_sent_loss) - torch.Tensor(sent_loss)

            if threshold is None:
                if ratio is not None:
                    threshold = self.get_threshold(diff_ppl, ratio, False)
                else:
                    raise ValueError("Either threshold or ratio must be provided")

            indices = torch.nonzero(diff_ppl > threshold, as_tuple=True)[0]
            _compressed_input_ids = [all_input_ids[context_id][i] for i in indices.tolist()]
            if len(_compressed_input_ids) != 0:
                _compressed_input_ids = torch.cat(_compressed_input_ids, dim=1)
            else:  # 如果一个句子都没选中，至少保留第一个
                _compressed_input_ids = all_input_ids[context_id][0]
            past_compressed_input_ids = _compressed_input_ids
            past_compressed_attention_mask = torch.ones_like(past_compressed_input_ids)
            compressed_input_ids.append(_compressed_input_ids)

            past_key_values = DynamicCache(
                tuple(
                    [
                        k[:, :, : end - context_len[context_id]],
                        v[:, :, : end - context_len[context_id]],
                    ]
                )
                for k, v in past_key_values
            )
            self_past_key_values = DynamicCache(
                tuple(
                    [
                        k[:, :, : end - context_len[context_id] - context_len[0]],
                        v[:, :, : end - context_len[context_id] - context_len[0]],
                    ]
                )
                for k, v in self_past_key_values
            )

        compressed_inputs = [self.tokenizer.decode(input_ids[0]) for input_ids in compressed_input_ids]
        return compressed_inputs

    def iterative_token_compress(
        self,
        prefix: str | List[str],
        context: List[str],
        ratio: float,
        ...
        prefixed_texts = prefix + context if isinstance(prefix, list) else [prefix] + context
        context_tokenized = [
            self.tokenizer(chunk, return_tensors="pt", add_special_tokens=False).to(self.device)
            for chunk in prefixed_texts
        ]
        context_len = [tokens["input_ids"].shape[1] for tokens in context_tokenized]
        input_ids = [t["input_ids"] for t in context_tokenized]

        attention_mask = [t["attention_mask"] for t in context_tokenized]
        past_key_values = None
        self_past_key_values = None
        end = 0
        compressed_input_ids, compressed_attention_mask = [], []
        past_compressed_input_ids = None  # 记录上一个压缩后的输入，和当前拼接
        past_compressed_attention_mask = None
        for context_id in range(len(context_tokenized)):
            end += context_len[context_id]
            if context_id == 0:
                response = self.model(
                    input_ids[0],
            ...
            if past_compressed_input_ids is not None and past_compressed_attention_mask is not None:
                past_compressed_cur_input_ids = torch.cat((past_compressed_input_ids, input_ids[context_id]), dim=-1)
                past_compressed_cur_attention_mask = torch.cat(
                    (past_compressed_attention_mask, attention_mask[context_id]), dim=-1
                )
            else:
                past_compressed_cur_input_ids = input_ids[context_id]
                past_compressed_cur_attention_mask = attention_mask[context_id]
            ...
            if context_len[context_id] == 1:
                # 如果当前块只有一个 token，则直接保留
                compressed_input_ids.append(input_ids[context_id])
                compressed_attention_mask.append(attention_mask[context_id])
                past_compressed_input_ids = input_ids[context_id]
                past_compressed_attention_mask = attention_mask[context_id]
                continue

            diff_ppl = self_loss[-(context_len[context_id] - 1) :] - loss[-(context_len[context_id] - 1) :]

            threshold = self.get_threshold(diff_ppl, ratio, False)

            _compressed_input_ids, _compressed_attention_mask = self.get_compress_input(
                diff_ppl, input_ids[context_id], attention_mask[context_id], threshold
            )
            past_compressed_input_ids = _compressed_input_ids
            past_compressed_attention_mask = _compressed_attention_mask
            compressed_input_ids.append(_compressed_input_ids)
            compressed_attention_mask.append(_compressed_attention_mask)
            end = end - context_len[context_id] + _compressed_input_ids.shape[1]
    ...
    def compress(
        ...
        threshold: float | None = None,
        compress_level="sentence",
    ) -> List[str]:
        if type(question) is list:
            prefix = question + [""]
        else:
            prefix = [question + instruction]
        if compress_level == "sentence":
            compressed_text = self.iterative_sent_compress(prefix, context, rate, threshold)
        else:
            if rate is None:
...
class PromptCompressor:
    ...
    def control_sentence_budget(
        ...
        if rank_method == "longllmlingua":
            sentence_ppl = [
                self.get_condition_ppl(sentence, question, condition_in_question).cpu().item()
    ...
    def segment_structured_context(
        ...
        for text in context:
            ...
            assert len(segments) == len(segs_rate) == len(segs_compress), (
                "The number of segments, rates, and compress flags should be the same."
            )
            assert all(seg_rate <= 1.0 for seg_rate in segs_rate), (
                "Error: 'rate' must not exceed 1.0. The value of 'rate' indicates compression rate and must be within the range [0, 1]."
            )
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                for j in range(ids.shape[0]):
                    ...
                    for word, word_prob in zip(words, word_probs):
                        if word_prob > threshold or (threshold == 1.0 and word_prob == threshold):
                            if (
                                drop_consecutive
